...
std::vector<int> getBestMove(UltimateTicTacToeBoard board, char rootPlayer, int maxTime) {
    ...
    for (int depth = 1; depth <= maxDepth; ++depth) {
        ...
        for(int moveIndex = 0; moveIndex < legalMoves.size(); ++moveIndex) {
            board.makeMove(legalMoves[moveIndex][0], legalMoves[moveIndex][1]);
            int eval = -negamax(board, depth - 1, 1, rootPlayer, -1, -beta, -alpha, endTime);
            board.unmakeMove(legalMoves[moveIndex][0], legalMoves[moveIndex][1]);
            if (eval > maxEval) {
                alpha = std::max(alpha, eval);
                maxEval = eval;
...
int oldNegamax(UltimateTicTacToeBoard board, int depth, int ply, char rootPlayer, int color, std::chrono::time_point<std::chrono::high_resolution_clock> endTime) {
    if (std::chrono::high_resolution_clock::now() > endTime) {
        return color * -9999;
    }
    ...
    if (depth == 0) {
        return color * oldEvaluate(board, rootPlayer);
    }
    int maxEval = -999;
    std::vector<std::vector<int>> legalMoves = board.getLegalMoves();
    std::vector<int> bestMove = legalMoves[0];
    ...
    for (int moveIndex = 0; moveIndex < legalMoves.size(); ++moveIndex) {
        ...
        board.makeMove(legalMoves[moveIndex][0], legalMoves[moveIndex][1]);
        int eval = -oldNegamax(board, depth - 1, ply + 1, rootPlayer, -color, endTime);
        board.unmakeMove(legalMoves[moveIndex][0], legalMoves[moveIndex][1]);
...
std::vector<int> oldGetBestMove(UltimateTicTacToeBoard board, char rootPlayer, int maxTime) {
    ...
    for (int depth = 1; depth <= maxDepth; ++depth) {
        ...
        for(int moveIndex = 0; moveIndex < legalMoves.size(); ++moveIndex) {
            board.makeMove(legalMoves[moveIndex][0], legalMoves[moveIndex][1]);
            int eval = -oldNegamax(board, depth - 1, 1, rootPlayer, -1, endTime);
            board.unmakeMove(legalMoves[moveIndex][0], legalMoves[moveIndex][1]);
            if (eval > maxEval) {
                maxEval = eval;
                bestMove = legalMoves[moveIndex];
